...
\begin{frame}{Motivation and Research Problem}
  \begin{columns}[T]
    \begin{column}{0.58\textwidth}
      \begin{block}{Motivation}
        Temperature changes in rocks can affect stress, deformation, and wave
        propagation. This is relevant to geothermal systems, underground
        engineering, and geophysical interpretation.
      \end{block}
      \vskip 3pt
      \begin{exampleblock}{Research problem}
        Estimate directional thermoelastic response using AI-assisted surrogate
        models under simplified effective-medium assumptions.
      \end{exampleblock}
      \vskip 2pt
      {\footnotesize\textbf{Scope:} research prototype for source--probe
      prediction, not field-validated industrial simulation.}
    \end{column}
    \begin{column}{0.38\textwidth}
      \centering
      \includegraphics[width=\linewidth,height=0.43\textheight,keepaspectratio]{img/basalt}
    \end{column}
  \end{columns}
\end{frame}

\begin{frame}{Aim, Tasks, and Scope}
  \begin{columns}[T]
    \begin{column}{0.58\textwidth}
      \begin{block}{Aim}
        Develop and justify an AI-guided framework for directional prediction of
        thermoelastic wave propagation in geological media.
      \end{block}
      \vskip 2pt
      {\footnotesize
      \begin{enumerate}
        \item Define the physical and geological basis.
        \item Formulate a simplified 2D thermoelastic prediction task.
        \item Prepare COMSOL-derived synthetic data.
        \item Compare PINN, FNO, MeshGraphNet, and Transformer-style routes.
        \item Interpret results, limitations, and future improvements.
      \end{enumerate}
      }
      \vskip 1pt
      {\scriptsize
      Object: thermoelastic wave propagation in geological media.\\
      Subject: AI-assisted source--probe directional prediction.}
    \end{column}
    \begin{column}{0.38\textwidth}
      \centering
      \includegraphics[width=\linewidth,height=0.47\textheight,keepaspectratio]{img/comsol_temperature_2d}
      \vskip 2pt
      {\scriptsize Controlled 2D thermoelastic field data support comparison.}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{COMSOL Temperature Field}
  \begin{columns}[T]
    ...
    \begin{column}{0.36\textwidth}
      \footnotesize
      \begin{block}{Thermal excitation}
        The heated source creates a localized high-temperature region inside
        the geological medium.
      \end{block}

      \begin{block}{Temperature distribution}
        The maximum temperature is concentrated near the source, while the
        surrounding domain remains close to the initial temperature.
      \end{block}

      \begin{block}{Dataset variable}
        Temperature is one of the main dynamic fields used for training and
        validating the neural models.
      \end{block}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{COMSOL Thermoelastic Response}
  \begin{columns}[T]
    \begin{column}{0.49\textwidth}
      ...
      \vskip 2pt
      \scriptsize Total displacement field
    \end{column}
    ...
    \begin{column}{0.49\textwidth}
      ...
      \vskip 2pt
      \scriptsize von Mises stress field
    \end{column}
  \end{columns}

  \vskip 4pt
  \footnotesize
  Local heating causes thermal expansion of the medium. As a result,
  displacement and stress are concentrated around the heated source, confirming
  the coupled thermoelastic behavior of the simulated process.
\end{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.42\textwidth}
      ...
      \begin{block}{Input / output}
        \begin{itemize}
          \item Input: $(x,y)$, time $t$, and material parameters
                $E,\nu,\rho,\alpha,k,C_p$.
          \item Output: temperature $T$ and in-plane displacement components
                $(u,v)$.
