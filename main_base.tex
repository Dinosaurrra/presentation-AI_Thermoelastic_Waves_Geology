  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.49\textwidth}
      ...
      \begin{tikzpicture}[
        ...
        \node[box] (ui) {Web UI\\HTML/CSS/JavaScript};
        \node[box, below=of ui] (api) {FastAPI backend\\validation + routing};
        \node[box, below=of api] (catalog) {Material catalog\\JSON rock presets};
        \node[box, right=16mm of api] (docker) {Docker Compose\\local services};

        \node[service, below=13mm of catalog, xshift=-42mm] (pinn) {PINN\\Python};
        \node[service, right=4mm of pinn] (fno) {FNO\\Python};
        \node[service, right=4mm of fno] (mgn) {MeshGraphNet\\Python};
        \node[service, right=4mm of mgn] (tr) {Transformer\\Python};

        \node[box, below=11mm of fno, xshift=31mm] (out) {Normalized response\\$T$, $u$, $v$, travel time};

        \draw[arrow] (ui) -- (api);
        \draw[arrow] (api) -- (catalog);
        \draw[arrow] (api.east) -- (docker.west);
        \draw[arrow] (api.south) -- ++(0,-6mm) -| (pinn.north);
        \draw[arrow] (api.south) -- ++(0,-6mm) -| (fno.north);
        \draw[arrow] (api.south) -- ++(0,-6mm) -| (mgn.north);
        \draw[arrow] (api.south) -- ++(0,-6mm) -| (tr.north);
        \draw[arrow] (pinn.south) |- (out.west);
        \draw[arrow] (fno.south) -- (out.north);
        \draw[arrow] (mgn.south) |- (out.east);
        \draw[arrow] (tr.south) |- (out.east);
      \end{tikzpicture}}
      \vspace{2mm}
...
\begin{frame}{Web Application Prototype}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.34\textwidth}
      ...
      \end{itemize}
      \vspace{0.5mm}
      {\scriptsize\textbf{Scientific scope.} Thesis-demo interface for
      reproducible comparison; not a field-validated thermoelastic simulator.}
    \end{column}
