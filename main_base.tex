...
\draw[->, thick] (conv.north) |- (spec.south);
...
\begin{columns}[T,onlytextwidth]
  ...
  \begin{column}{0.58\textwidth}
    ...
    \begin{tikzpicture}[
      ...
      % Node features
      \node[node] (n1)
        {$\mathbf{x}_1=(\mathbf{p}_1,\mathbf{s}_1^t,\mathbf{m}_1,\mathbf{c})$};

      \node[node, below=3mm of n1] (n2)
        {$\mathbf{x}_2=(\mathbf{p}_2,\mathbf{s}_2^t,\mathbf{m}_2,\mathbf{c})$};

      \node[below=1mm of n2] (dots)
        {$\vdots$};

      \node[node, below=1mm of dots] (nN)
        {$\mathbf{x}_N=(\mathbf{p}_N,\mathbf{s}_N^t,\mathbf{m}_N,\mathbf{c})$};

      % Edge attributes
      \node[edgebox, right=10mm of n1.east] (e1)
        {$\mathbf{e}_{12}=(\Delta x,\Delta y,\Delta z,\|\Delta\mathbf{p}\|)$};

      \node[edgebox, below=3mm of e1] (e2)
        {$\mathbf{e}_{ij}=\text{relative geometry}$};

      \node[below=1mm of e2] (edots)
        {$\vdots$};

      \node[edgebox, below=1mm of edots] (eM)
        {$\mathbf{e}_{M}$};
      ...
      \node[
        ...
        fit=(n1)(nN),
        inner sep=3mm,
        label={[font=\scriptsize]above:Node features}
      ] {};
      ...
      \node[
        ...
        fit=(e1)(eM),
        inner sep=3mm,
        label={[font=\scriptsize]above:Edge attributes}
      ] {};
  ...
  \begin{column}{0.40\textwidth}
    ...
    \begin{block}{Graph representation}
      ...
      The COMSOL mesh is converted into a graph:
      nodes contain physical states, while edges describe spatial relations.
    \end{block}
    ...
    \begin{block}{Node input}
      ...
      \[
        \mathbf{x}_i=(\mathbf{p}_i,\mathbf{s}_i^t,\mathbf{m}_i,\mathbf{c})
      \]
      where $\mathbf{p}_i$ is position, $\mathbf{s}_i^t$ is the current physical
      state, $\mathbf{m}_i$ are material parameters, and $\mathbf{c}$ is the
      scenario condition.
    \end{block}
    ...
    \begin{block}{Output}
      ...
      The model predicts the next physical fields for every mesh node.
    \end{block}
