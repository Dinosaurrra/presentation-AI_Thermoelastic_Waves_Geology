...
\begin{frame}{PINN Input and Architecture}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{block}{Baseline MLP}
        \[
          X\rightarrow\mathrm{MLP}_{\theta}(X)\rightarrow\widehat{Y}
        \]
      \end{block}
      \scriptsize
      Plain fully connected baseline kept for comparison and checkpoint
      compatibility.
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{block}{ResSplitPINN}
        \[
          [x,y,z,t]\rightarrow E_c,\qquad
          [E,\nu,\rho,\alpha,k,C_p]\rightarrow E_m
        \]
        \[
          [E_c,E_m]\rightarrow\mathrm{trunk}\rightarrow
          (\hat{T},\hat{u},\hat{v},\hat{w})
        \]
      \end{block}
      \scriptsize
      Coordinates and material parameters are encoded separately, then fused and
      decoded by split temperature and displacement heads.
    \end{column}
  \end{columns}
\end{frame}
